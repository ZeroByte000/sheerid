...
- Playwright (and Chromium) — used to render the HTML template as an accurate PNG
...
You can run the verifier from the command line. Provide either the full SheerID URL with `--url` or just the verification ID as a positional argument. If you omit them, the script will prompt interactively after it starts.
...
- Verification ID or URL (required)
- Name (optional)
- Email (optional)
- Resend verification email (y/N)

---

## 🧩 Troubleshooting

- Missing module errors (e.g., `ModuleNotFoundError: No module named 'xhtml2pdf'`) → run `python3 -m pip install xhtml2pdf`.
...
- If you don’t receive the verification email (verification stuck in `emailLoop`), try running the script with the `--resend-email` flag to have the tool resend the verification email and wait for the server to progress to the `docUpload` step.
...
If you want, I can also add a `--no-save` flag to skip writing PNGs to disk or implement PDF saving to `image/` as well—tell me which you prefer.
